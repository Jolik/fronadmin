...
type
  ...
  TSheduleList = class(TEntityList)
  private
    FDisabled: boolean;
  ...
  public
    function Assign(ASource: TSheduleList): boolean;
...
function TSheduleList.Assign(ASource: TSheduleList): boolean;
begin
  Result := false;
  ...
  begin
    var s := TShedule.Create;
    if not s.Assign(Items[i]) then
      exit;
    Add(s);
  end;
